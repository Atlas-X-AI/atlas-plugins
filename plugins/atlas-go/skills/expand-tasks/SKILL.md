...
Do NOT activate for: single task research (use /research-before-coding), PRD generation (use /atlas-go:go).
...
| `tasks.json` not found | Exit skill, tell user to run `/atlas-go:go` first |
...
```
/atlas-go:go → generates PRD → parses into tasks
    ↓
