...
3. Route via Skill tool:
   ...
   - current_phase is GENERATE → invoke `/atlas-go:generate`
   - current_phase is HANDOFF → invoke `/atlas-go:handoff`
   - current_phase is EXECUTE → invoke `/atlas-go:execute-task`
